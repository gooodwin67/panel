...
export class GuiClass {
  ...
  refreshLight() {
    ...
    const ctrlPosX = posFolder.add(pointLight.position, 'x', -10, 10, 0.01).name('X').listen().onChange(syncBulb);
    const ctrlPosY = posFolder.add(pointLight.position, 'y', -10, 10, 0.01).name('Y').listen().onChange(syncBulb);
    const ctrlPosZ = posFolder.add(pointLight.position, 'z', -10, 10, 0.01).name('Z').listen().onChange(syncBulb);
  
    this.lightControllers.push(ctrlPosX, ctrlPosY, ctrlPosZ);
  
    // Параметры света
    this.lightControllers.push(
      this.lightFolder.add(pointLight, 'intensity', 0, 50, 0.1).name('Яркость').listen(),
      this.lightFolder.add(pointLight, 'distance', 0, 50, 0.1).name('Дальность').listen(),
      this.lightFolder.add(pointLight, 'decay', 0, 5, 0.01).name('Затухание').listen()
    );
  
    // Цвет света
    this.lightControllers.push(
      this.lightFolder.addColor(lightParams, 'lightColor').name('Цвет света').onChange((value) => {
    ...
    this.lightControllers.push(
      this.lightFolder.add(lightParams, 'kelvin', 1000, 12000, 50).name('Температура (K)').onChange((value) => {
    ...
    this.lightControllers.push(
      bulbFolder.add(lightParams, 'bulbVisible').name('Виден').onChange((value) => {
        bulbMesh.visible = value;
      }),
      bulbFolder.addColor(lightParams, 'bulbColor').name('Цвет').onChange((value) => {
    ...
    this.lightControllers.push(
      shadowFolder.add(lightParams, 'castShadow').name('castShadow').onChange((value) => {
        pointLight.castShadow = value;
      }),
      shadowFolder.add(lightParams, 'shadowMapSize', [256, 512, 1024, 2048]).name('mapSize').onChange((value) => {
        const size = Number(value);
        pointLight.shadow.mapSize.set(size, size);
        pointLight.shadow.needsUpdate = true;
      }),
      shadowFolder.add(lightParams, 'shadowBias', -0.01, 0.01, 0.00001).name('bias').onChange((value) => {
  ...
  refreshAmbient() {
    ...
    this.ambientControllers.push(
      this.ambientFolder.add(ambientLight, 'intensity', 0, 5, 0.01).name('Яркость').listen(),
      this.ambientFolder.addColor(ambientParams, 'ambientColor').name('Цвет').onChange((value) => {
